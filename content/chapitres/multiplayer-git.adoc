...
[.small]
Cela reste des outils, ça ne résouds pas tout non plus.
...
** `rw`: Le "response writer", qui permet d'écrire le code de status, les headers et le corps de la réponse
...
* Extraire l'identifiant (la valeur `id`) du path en utilisant la méthode link:https://pkg.go.dev/net/http#Request.PathValue[PathValue].
* Parser la valeur obtenue en `int64` en utilisant link:https://pkg.go.dev/strconv#ParseInt[strconv.ParseInt]
* Appeler la methode `Delete` du `VehicleStore` en passant l'identifiant et le "contexte" récupéré en appelant `r.Context()`
...
** Si le véhicle indiqué n'existe pas, répondre 404.
* Il convient de gérer les erreurs correctement, vous pouvez vous inspirer des hautres handlers défini dans le package!
