...
A vous de jouer: Corrigez la fonctionnalité "suppression d'un vehicule" dans projet de votre binôme
...
* Le code à corriger se trouve dans le fichier `vehicle/delete.go`, la methode `ServeHTTP` du `DeleteHandler` ne fait que retourner une erreur.
* Ce qu'il faut faire:
** Extraire l'identifiant depuis le path en utilisant la méthode link:https://pkg.go.dev/net/http#Request.PathValue[PathValue] de l'instance de `http.Request` passée en argument. Le nom de la valeur à extraire est `id`.
** Parser la valeur obtenue en `int64` en utilisant link:https://pkg.go.dev/strconv#ParseInt[strconv.ParseInt]
** Appeler la methode `Delete` du `VehicleStore` avec cet identifiant et le "contexte" récupéré en appelant `r.Context()`
** Enfin si la suppression est réussie, répondre un status code 204 en appelant `rw.WriteHeader(http.StatusNoContent)`
...
💡 Astuce: link:https://github.com/franckverrot/clamav-client/pull/12#discussion_r526222319[Proposez des solutions,window="_blank"] plutôt que simplement pointer les problèmes.
