...
* Beaucoup d'opportunités professionelles autour de ce langage
...
* La (link:https://pkg.go.dev/std[très fournie]) librarie standard de Go
...
* 🎓 Go permets de *référencer* cet emplacement memoire en copiant son adresse dans une autre variable avec l'opérateur `&`. Autrement dit, on *crée un pointeur*.
...
* Mais `type` peut être appliqué á des tas d'autre choses.
...
* Le premier argument avant le nom est appelé *receveur*, c'est la référence vers l'objet sur lequel la méthode est appellée
...
* On utilise une valeur quand on souhaite que la méthode ne puisse pas changer l'objet sur laquelle elle est appellée
...
* Et on déserialise la réponse dans un object pour accéder au `climat`
...
* On peut concaténer des objets à une slice avec l'operateur `append`
...
* Ecrivez une fonction `WordCount` en go qui accepte une chaine de caractère et qui retourne le nombre d'occurences de chacun des mots contenu dans la chaine.
