...
* Si votre dépendance ne fonctionne plus ou est compromise, votre app sera impactée
* Attention à ne pas rajouter une dépendance trop grosse pour n'utiliser qu'une petite fonctionalité!
...
** 👀 le code. Est-ce que vous le comprenez? Est-ce que vous pourriez le débugger ou le faire vous même?
...
** Construit un graph de dépendences de façon et le résoud a obtenir une liste de dépendances à télecharger de façon déterministe
...
** ✅ La version est controllée par la version de go utilisée pour compiler
* La base de code du projet
** ✅ Controllée par notre VCS
* D'une ou plusieurs librairies externes
** ❌ Pas de solutions pour l'instant

== Bonjour go modules

* Go introduit la notion de *modules*: une collection de packages qui sont livrés, versionnés et distribués enssemble
...
* ⚠️ C'est verbeux, vous pouvez utiliser `| grep` pour filtrer le resultat

* Et pour finir, on oublie pas de créer un commit :)

== Résolution Reproductibilite: l'algorithme MVS (1/2)

image:buildlist.svg[caption="BuildGraph",width=700]

== Résolution Reproductibilible: l'algorithme MVS (2/2)
...
== Controller le Contenu Téléchargé
...
* `go` calcule ensuite une somme de contrôle (checksum) des fichiers téléchargés et la compare avec un référenciel établi lors du premier télechargement: le fichier `go.sum`.
** ➡️  Cela garantit que les dépendances téléchargées n'ont pas été altérées entre deux télechargements.
...
* `go get -u ./...` Update tous les modules dépendants
...
* Une bonne compréhension de son système de gestion de dépendances est nécessaire pour garantir la reproductibilité de nos builds
