...
== Prérequis: Ordonancement et Execution Contionelle des Jobs

* Un workflow execute par défaut tous les jobs qui le compose en parallèle
* Il est possible de conditoner l'execution d'un job à la terminaison d'un autre à l'aide du mot clé `needs` (link:https://docs.github.com/en/actions/learn-github-actions/workflow-syntax-for-github-actions#jobsjob_idneeds[doc])
* Il est possible d'executer conditionellement un job ou un step à l'aide du mot clé `if` (link:https://docs.github.com/en/actions/learn-github-actions/workflow-syntax-for-github-actions#jobsjob_idif[doc])
