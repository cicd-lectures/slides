...
* 👷🏽‍♀️ C'est à vous de mettre à jour le workflow pour ajouter un 2nd job nommé `runMaven` dans le workflow :
...
👷🏽‍♀️ C'est à vous de modifier le projet "menu-server" pour avoir 1 job nommé `build` dans un workflow nommé "CI" (fichier `ci.yaml`) qui va :
