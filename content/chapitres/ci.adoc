...
* Dans le répertoire `/workspaces/menu-server`
...
* Commitez puis poussez sur la branch `main`
...
// TODO: insert Interstellar meme here
...
👷🏽‍♀️ C'est à vous de modifier le projet "menu-server" pour avoir 1 workflow nommé `ci.yaml` portant un job nommé `build` qui va sur un évenement de push:
...
(vous pouvez supprimer votre ancien workflow `bonjour.yml`)
