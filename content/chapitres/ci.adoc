...
** https://github.com/marketplace/actions/neo-cowsay[GitHub Action for cowsay]
** https://github.com/marketplace/actions/file-reader[GitHub Action to read a file to an `output`]
** https://docs.github.com/en/actions/learn-github-actions/contexts#steps-context[Documentation de `steps.outputs`]
...
👷🏽‍♀️ C'est à vous de modifier le projet pour faire l'intégration continue du code Java de "menu-server".

A chaque commit poussé sur votre dépôt GitHub, un workflow va :

* Récupérer le code de l'application depuis GitHub
* S'assurer que les *même* versions de Java et Maven sont installées
** 💡 https://github.com/marketplace/actions/setup-maven-action
* L'application est compilée
* Le `jar` de l'application est fabriqué

[.small]
* Pensez à supprimer le workflow `bonjour.yaml`
